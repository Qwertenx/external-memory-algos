...
private:
	...
	void insert(std::shared_ptr<Node> node, const TKey& key, const TValue& value);
	...
	std::pair<TKey, TValue> removeMin(std::shared_ptr<Node> node);
	void erase(std::shared_ptr<Node> node, const TKey& key);
	TValue find(std::shared_ptr<Node> node, const TKey& key) const;
	bool update(std::shared_ptr<Node> node, const TKey& key, const TValue& value);
...
public:
	BTreeMap(int parameter, bool binary, std::string storageLocation);
	void insert(std::pair<TKey, TValue> keyValuePair);
	bool update(std::pair<TKey, TValue> keyValuePair);
	void erase(TKey key);
...
template<typename TKey, typename TValue>
BTreeMap<TKey, TValue>::BTreeMap(int parameter, bool binary, std::string storageLocation)
{
	FILE* input;
...
template<typename TKey, typename TValue>
BTreeMap<TKey, TValue>::~BTreeMap()
{
	backupTree();
}

template<typename TKey, typename TValue>
std::string BTreeMap<TKey, TValue>::getTempFileName(const std::string& storageLocation, int index) const
{
	return (storageLocation + "/node" + std::to_string(index) + ".bin");
}

template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::backupTree() const
{
	FILE* output;
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::readNode(std::shared_ptr<Node> &node, int index) const
{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::writeNode(std::shared_ptr<Node> node)
{
	...
	if (binary)
	{
		...
		fwrite(node->values, sizeof(TValue), node->size, fileOut);
		if (!node->leaf)
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::createNode(std::shared_ptr<Node> &node)
{
	node = std::shared_ptr<Node>(new Node());
	node->size = 0;
...
template<typename TKey, typename TValue>
std::shared_ptr<typename BTreeMap<TKey, TValue>::Node> BTreeMap<TKey, TValue>::split(std::shared_ptr<Node> parent, std::shared_ptr<Node> childPtr, int child)
{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::merge(std::shared_ptr<Node> parent, std::shared_ptr<Node> firstPart, std::shared_ptr<Node> secondPart, int firstPartIndex, bool direction)
{
	if (direction)
	{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::keyRepositioning(std::shared_ptr<Node> parent, std::shared_ptr<Node> firstPart, std::shared_ptr<Node> secondPart, int firstPartIndex, bool direction)
{
	if (direction)
	{
		firstPart->keys[firstPart->size] = parent->keys[firstPartIndex];
...
template<typename TKey, typename TValue>
bool BTreeMap<TKey, TValue>::eraseResolve(std::shared_ptr<Node> parent, std::shared_ptr<Node> child, int childIndex)
{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::removeFromLeaf(std::shared_ptr<Node> leaf, int keyIndex, bool rootCall)
{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::removeFromInternal(std::shared_ptr<Node> node, int keyIndex, bool rootCall)
{
	...
	std::pair<TKey, TValue> result = removeMin(child);
...
template<typename TKey, typename TValue>
std::pair<TKey, TValue> BTreeMap<TKey, TValue>::removeMin(std::shared_ptr<Node> node)
{
	if (node->leaf)
	{
		std::pair<TKey, TValue> result = { node->keys[0], node->values[0] };
...
template<typename TKey, typename TValue>
TValue BTreeMap<TKey, TValue>::find(std::shared_ptr<Node> node, const TKey& key) const
{
...
template<typename TKey, typename TValue>
bool BTreeMap<TKey, TValue>::update(std::shared_ptr<Node> node, const TKey& key, const TValue& value)
{
	int keyIndex = std::upper_bound(node->keys, node->keys + node->size, key) - node->keys - 1;
	if (keyIndex != -1 && node->keys[keyIndex] == key)
	{
	...
	std::shared_ptr<Node> child;
	readNode(child, node->children[keyIndex + 1]);
	return update(child, key, value);
}

template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::insert(std::shared_ptr<Node> node, const TKey& key, const TValue& value)
{
	int keyIndex;
	if (node->leaf)
	{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::erase(std::shared_ptr<Node> node, const TKey& key)
{
	int keyIndex;
	keyIndex = std::upper_bound(node->keys, node->keys + node->size, key) - node->keys - 1;
	if (keyIndex != -1 && node->keys[keyIndex] == key)
...
template<typename TKey, typename TValue>
TValue BTreeMap<TKey, TValue>::find(TKey key) const
{
...
template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::insert(std::pair<TKey, TValue> keyValuePair)
{
	std::shared_ptr<Node> root;
	readNode(root, rootIndex);
	if (root->size == 2 * parameter - 1)
	{
		...
		insert(newRoot, keyValuePair.first, keyValuePair.second);
	}
	else
		insert(root, keyValuePair.first, keyValuePair.second);
	backupTree();
}

template<typename TKey, typename TValue>
void BTreeMap<TKey, TValue>::erase(TKey key)
{
	std::shared_ptr<Node> root;
	readNode(root, rootIndex);
	int keyIndex;
...
template<typename TKey, typename TValue>
bool BTreeMap<TKey, TValue>::update(std::pair<TKey, TValue> keyValuePair)
{
	std::shared_ptr<Node> root;
	readNode(root, rootIndex);
	return update(root, keyValuePair.first, keyValuePair.second);
}
